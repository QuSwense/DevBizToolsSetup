...
-- Tracks full historical snapshots of changes across all entity tables
-- ==========================================
IF NOT EXISTS (
    SELECT * 
    FROM [sysobjects] 
...
BEGIN
    CREATE TABLE [EntityChangeHistory] (
        ...
        [TableName] NVARCHAR(100) NOT NULL,    -- Target Table (e.g., 'SoapApplications', 'SoapRequestFiles')
        [TableId] NVARCHAR(100) NOT NULL,      -- Target Primary Key (e.g., AppId or FileId)
        [ActionType] VARCHAR(20) NOT NULL CHECK ([ActionType] IN ('CREATE', 'UPDATE', 'DELETE')),
        [OldValuesJson] NVARCHAR(MAX) NULL,    -- JSON snapshot of record before change
        [NewValuesJson] NVARCHAR(MAX) NULL,    -- JSON snapshot of record after change
...
-- Logs overall status and execution metadata for a run
-- ==========================================
IF NOT EXISTS (
...
-- ==========================================
-- Create SoapResponseFiles Table (SEPARATE RESPONSE STORAGE)
-- Raw text response modified in C# to substitute extracted blobs with [fileTable:Id]
-- ==========================================
IF NOT EXISTS (
...
BEGIN
    CREATE TABLE [SoapResponseFiles] (
        [Id] INT IDENTITY(1,1) PRIMARY KEY,
        [ExecutionHistoryId] INT NOT NULL FOREIGN KEY REFERENCES [SoapExecutionHistory]([Id]) ON DELETE CASCADE,
        [ResponseFormat] VARCHAR(10) CHECK ([ResponseFormat] IN ('XML','JSON','PDF','BINARY')),
        [FileData] VARBINARY(MAX) NOT NULL,       -- Main response file data (compressed/encrypted in C#)
        [UncompressedSizeBytes] INT NULL,
