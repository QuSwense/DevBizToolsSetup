...
public partial class WsdlSync
{
    ...
    // ═══════════════════════════════════════════════════════════
    // WSDL comparison content — no longer sourced from mock JSON.
    // TODO: derive from MSSQL (WsdlVersions.Content) once the schema exists.
    // ═══════════════════════════════════════════════════════════

    private string _previousWsdlContent = "";
    private string _changedWsdlContent = "";
    /// <summary>Available WSDL file names for the template editor dropdown.</summary>
    private string[] _wsdlFileNames = [];

    /// <summary>
    /// Performs WSDL comparison and updates change detection state.
    /// The previous/changed content is no longer sourced from mock JSON; with no database
    /// table for it yet the comparison runs against empty content.
    /// </summary>
    ...
    private void OpenTemplateEditor(string? templateId)
    {
        ...
        if (templateId is not null)
        {
            ...
            if (tpl is not null)
            {
                ...
                // Content stores the referenced WSDL file name. The WSDL body previously
                // came from mock JSON; not yet DB-backed. TODO: load from MSSQL
                // (WsdlVersions.Content) once the schema exists.
        ...
        else
        {
            ...
            _editTplDescription = "";
            _editTplExtendsId = "";
            _editTplWsdlFile = _wsdlFileNames.FirstOrDefault() ?? "";
            // WSDL file content previously came from mock JSON; not yet DB-backed.
            _editTplContent = "";
        }
        _showTemplateEditor = true;
    }

    private void OnTemplateWsdlFileChanged(ChangeEventArgs e)
    {
        _editTplWsdlFile = e.Value?.ToString() ?? "";
        // WSDL file content previously came from mock JSON; not yet DB-backed.
        // TODO: load from MSSQL (WsdlVersions.Content) once the schema exists.
        _editTplContent = "";
        StateHasChanged();
    }
    ...
    private void HandleSaveTemplate()
    {
        ...
        // Content is a WSDL file reference. Previously the edited content was written back
        // to a mock JSON file; there is no DB table for it yet.
        // TODO: persist to MSSQL (WsdlVersions.Content) once the schema exists.
    ...
    private Task OpenCreateFromTemplate(string templateId)
    {
        ...
        if (template is not null)
        {
            _requestFileName = "";
            _requestVarValues = [];
            // Content stores the referenced WSDL file name. The WSDL body previously came
            // from mock JSON; not yet DB-backed.
            // TODO: load from MSSQL (WsdlVersions.Content) once the schema exists.
            _createRequestTemplateContent = "";
    ...
    private async Task DownloadWsdl(string content, string fileName)
    {
        try
        {
            var module = await _js.InvokeAsync<IJSObjectReference>(
                "import", "./_content/OrbitHub.SoapApplications/js/download.js");
            try
            {
