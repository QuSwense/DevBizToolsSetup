...
public partial class RequestFiles : IDisposable
{
    ...
    /// <summary>
    /// Simulated network delay for the loading skeleton. The old MockDb:RequestFilesDelayMs
    /// config key was removed with the mock database; returns 0 (no artificial delay).
    /// </summary>
    ...
    /// <summary>
    /// Persists the in-memory file list. Request files are not yet backed by a database
    /// table — previously saved to mock JSON. TODO: persist to MSSQL (SoapRequestFiles)
    /// once the schema exists.
    /// </summary>
    private Task PersistFilesAsync()
    {
