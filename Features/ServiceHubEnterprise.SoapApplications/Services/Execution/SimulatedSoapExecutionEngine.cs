...
public class SimulatedSoapExecutionEngine : IExecutionEngine
{
    ...
    /// <summary>Delay (ms) between stages, from MockDb:ExecutionStageDelayMs (default 500).</summary>
    private int StageDelayMs
    {
        get
        {
            var raw = _configuration["MockDb:ExecutionStageDelayMs"];
            return int.TryParse(raw, out var ms) ? ms : 500;
        }
    }
