...
& structure principles established in the `ServiceHubEnterprise.Dashboard` redesign, and
...
- [ ] **Map current architecture** of the feature:
  ...
  - Whether the feature already references `Components/ServiceHubEnterprise.Grid` (see Phase 2)
...
**Shared table component** (already in the solution — `ServiceHubEnterprise.Grid`):
...
- [ ] Add a `ProjectReference` to `Components/ServiceHubEnterprise.Grid/ServiceHubEnterprise.Grid.csproj`
...
- [ ] **Promote** the 5 Dashboard widgets into a shared components project (e.g., extend
      `ServiceHubEnterprise.Grid` or add `ServiceHubEnterprise.Ui`) so every feature references one copy (recommended)
...
See `Features/ServiceHubEnterprise.SoapApplications/Pages/Applications.razor` for a full example
...
- **Cross-feature references**: self-contained `LoadJsonAsync<T>` reads from `mock_db` are the
  norm, but referencing the shared `ServiceHubEnterprise.Grid` project is expected for tables.
