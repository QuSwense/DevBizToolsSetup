    ...
    SELECT
        ...
        CAST(
            CASE
                ...
                -- ── Foreign key (generic catch-all — must stay before specific FK names) ──
                ...
                WHEN column_object.name = N'HttpMethod' THEN N'HTTP method used by the REST operation (GET, POST, PUT, DELETE, etc.).'
                ...
                -- ── SOAP namespace columns ─────────────────────────────────────────────────
                WHEN column_object.name = N'DetailContent' THEN N'JSON array of namespace prefix and URI pairs declared by the SOAP operation schema.'
                ...
                WHEN column_object.name = N'OutputDataTypeId' THEN N'Identifier of the RuleSetContextObjects record that defines the output data type of this rule set.'
                WHEN column_object.name = N'InputOrOutputType' THEN N'Indicates whether the linked context object is an input or output type for the rule set (Input or Output).'

                -- ── Permission / role columns ──────────────────────────────────────────────
                ...
                -- ── File format / compression columns ─────────────────────────────────────
                WHEN column_object.name = N'FileFormat' THEN N'Format of the stored file payload, such as XML, JSON, PDF, or BINARY.'
                WHEN column_object.name = N'ContentFormat' THEN N'Format of the stored binary content, such as XML, JSON, PDF, or BINARY.'
                WHEN column_object.name = N'CompressionAlgorithmType' THEN N'Algorithm used to compress the stored content (e.g., Zstandard, Brotli, Gzip, or none).'
                ...
                WHEN column_object.name = N'UncompressedSizeBytes' THEN N'Size of the content before compression, in bytes.'
                ...
                -- ── Binary embedding store columns ─────────────────────────────────────────
                WHEN column_object.name = N'KeyName' THEN N'Reference key name used to identify the binary asset within its parent XML or JSON file.'
                WHEN column_object.name = N'AdditionalDetails' THEN N'Optional JSON document containing supplementary metadata for the binary content.'

                -- ── Delta / snapshot columns ───────────────────────────────────────────────
                ...
                -- ── Element indexing columns ───────────────────────────────────────────────
                WHEN column_object.name = N'ElementName' THEN N'Name or key of the indexed element within the file.'
                WHEN column_object.name = N'ElementType' THEN N'Source type of the element, such as Text, Image, Table, or Field (PDF) or XML/JSON.'
                WHEN column_object.name = N'ElementValue' THEN N'Scalar value of the indexed element.'
                WHEN column_object.name = N'ValueType' THEN N'Data type of the element value (e.g., String, Number, Boolean, Array, Object, Null).'
                WHEN column_object.name = N'DataType' THEN N'C#-mapped data type of the element or setting value.'
                WHEN column_object.name = N'XmlPath' THEN N'XPath expression identifying the location of the indexed XML element.'
                WHEN column_object.name = N'JsonPath' THEN N'JSONPath expression identifying the location of the indexed JSON element.'
                ...
                -- ── Indexing FK columns ────────────────────────────────────────────────────
                WHEN column_object.name = N'IndexingXmlFileElementId' THEN N'Identifier of the related IndexingXmlFileElements record.'
                WHEN column_object.name = N'IndexingJsonFileElementId' THEN N'Identifier of the related IndexingJsonFileElements record.'
                WHEN column_object.name = N'IndexingPdfFileElementId' THEN N'Identifier of the related IndexingPdfFileElements record.'
                WHEN column_object.name = N'IndexingXmlFileElementValueId' THEN N'Identifier of the related IndexingXmlFileElementValues record.'
                WHEN column_object.name = N'IndexingJsonFileElementValueId' THEN N'Identifier of the related IndexingJsonFileElementValues record.'
                WHEN column_object.name = N'IndexingPdfFileElementValueId' THEN N'Identifier of the related IndexingPdfFileElementValues record.'
                ...
                WHEN column_object.name = N'BinaryEmbeddingsStoreId' THEN N'Identifier of the related BinaryEmbeddingStores record.'
                WHEN column_object.name = N'RequestFileId' THEN N'Identifier of the related ServiceRequestFiles record.'
                WHEN column_object.name = N'ResponseFileId' THEN N'Identifier of the related ServiceResponseFiles record.'
                ...
                WHEN column_object.name = N'IndexingFailureReason' THEN N'Error message and stack trace captured when background indexing fails.'
                WHEN column_object.name = N'LastIndexedAt' THEN N'Date and time at which background element parsing last completed successfully.'

                -- ── User activity columns ──────────────────────────────────────────────────
                WHEN column_object.name = N'ActivityType' THEN N'Category of activity performed by the user, such as Login or FeatureUsage.'
                ...
                -- ── Settings columns ───────────────────────────────────────────────────────
                WHEN column_object.name = N'Category' THEN N'Category used to group the global setting (e.g., General, UI, Authentication).'
                WHEN column_object.name = N'SettingKey' THEN N'Unique key that identifies the setting.'
                WHEN column_object.name = N'SettingValue' THEN N'Configured value of the setting, stored as text to accommodate various data types.'
                WHEN column_object.name = N'IsUserOverridable' THEN N'Indicates whether users may override this global setting with a user-specific value.'

                -- ── Execution tracking columns ─────────────────────────────────────────────
                WHEN column_object.name = N'ExecutionOrder' THEN N'Ordinal position that determines the sequence in which this item is executed within its parent group or suite.'
                WHEN column_object.name = N'ExecutionStatus' THEN N'Current or final status of the execution: Pending, InProgress, Completed, or Failed.'
                WHEN column_object.name = N'ExecutionDetails' THEN N'JSON or text details captured during execution, including logs and error messages when applicable.'
                WHEN column_object.name = N'ExecutionTimeMs' THEN N'Total execution duration in milliseconds.'
                ...
                WHEN column_object.name = N'ExecutedBy' THEN N'UserId of the user who initiated the execution.'
                WHEN column_object.name = N'IsSuccess' THEN N'Indicates whether the execution completed successfully (1) or not (0).'
                WHEN column_object.name = N'ErrorMessage' THEN N'Error message captured when execution fails.'

                -- ── HTTP execution detail columns ──────────────────────────────────────────
                WHEN column_object.name = N'HttpRequestUrl' THEN N'Full URL used for the HTTP request, including any query-string parameters appended for GET calls.'
                WHEN column_object.name = N'HttpRequestMethod' THEN N'HTTP method used for the request (e.g., GET, POST, PUT, DELETE).'
                WHEN column_object.name = N'HttpStatusCode' THEN N'HTTP status code returned by the service (e.g., 200, 400, 500).'
                WHEN column_object.name = N'HttpVersion' THEN N'HTTP protocol version used for the request and response (e.g., HTTP/1.1, HTTP/2).'
                WHEN column_object.name = N'HttpRequestDurationMs' THEN N'Total round-trip HTTP request duration in milliseconds.'
                WHEN column_object.name = N'HttpRequestHeaders' THEN N'JSON document containing the HTTP request headers sent to the service.'
                WHEN column_object.name = N'HttpResponseHeaders' THEN N'JSON document containing the HTTP response headers received from the service.'
                WHEN column_object.name = N'HttpContentType' THEN N'Content-Type header value returned by the service.'
                WHEN column_object.name = N'HttpContentLength' THEN N'Content-Length of the HTTP response body, in bytes.'

                -- ── Database audit columns (ServiceResponseFilesDatabaseAudits) ─────────────
                WHEN column_object.name = N'AutheticationName' THEN N'Name of the database authentication context or connection used during the database operation.'
                WHEN column_object.name = N'Details' THEN N'JSON or text log details capturing the outcome of the database operation.'
                WHEN column_object.name = N'ExtractedAt' THEN N'Date and time at which the database audit record was extracted or written.'

                -- ── Record lifecycle / concurrency columns ─────────────────────────────────
                WHEN column_object.name = N'RecordVersion' THEN N'Application-managed version string (format YY.QQ.NN) used for optimistic concurrency control and change tracking.'
                WHEN column_object.name = N'IsActive' THEN N'Indicates whether this record is active and available for use (1) or soft-deleted (0).'

                -- ── UI / navigation columns ────────────────────────────────────────────────
                WHEN column_object.name = N'IsVisibleInNav' THEN N'Indicates whether this page or feature is shown in the navigation menu.'
                WHEN column_object.name = N'FeatureFlag' THEN N'Name of the feature flag that controls whether this page or feature is visible.'
                WHEN column_object.name = N'ActionName' THEN N'Internal name of the UI action, such as Create, Edit, or Delete.'
                WHEN column_object.name = N'DisplayName' THEN N'Human-readable label displayed for this UI action or item.'
                WHEN column_object.name = N'UiElementId' THEN N'CSS selector or UI identifier of the element that triggers this action.'
                ...
                WHEN column_object.name = N'CreatedBy' THEN N'UserId of the user who created this record.'
                ...
                WHEN column_object.name = N'LastUpdatedBy' THEN N'UserId of the user who last updated this record.'
