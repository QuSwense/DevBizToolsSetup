/*
    Stored Procedure: usp_InsertIndexingJsonFileElementMappings
    Description: Inserts a mapping between a JSON file element search entry and a request or response file.
    Validates that exactly one of RequestFileId or ResponseFileId is provided.
*/
CREATE PROCEDURE [dbo].[usp_InsertIndexingJsonFileElementMappings]
    @IndexingJsonFileElementSearchId BIGINT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM [dbo].[IndexingJsonFileElementSearch] WHERE [Id] = @IndexingJsonFileElementSearchId)
        BEGIN
            RAISERROR('IndexingJsonFileElementSearch with Id %I64d not found.', 16, 1, @IndexingJsonFileElementSearchId);
            IF @LocalTranStarted = 1 AND @@TRANCOUNT > 0
                ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        BEGIN
            ...
            SELECT
                ...
                [RequestFileId],
                [ResponseFileId],
                [IndexingJsonFileElementSearchId],
                0 AS IsNew
            FROM [dbo].[IndexingJsonFileElementMappings]
            WHERE [IndexingJsonFileElementSearchId] = @IndexingJsonFileElementSearchId
              AND ((@RequestFileId IS NOT NULL AND [RequestFileId] = @RequestFileId)
                   OR (@ResponseFileId IS NOT NULL AND [ResponseFileId] = @ResponseFileId));
        ...
        -- Insert new mapping
        INSERT INTO [dbo].[IndexingJsonFileElementMappings] (
            [RequestFileId],
            [ResponseFileId],
            [IndexingJsonFileElementSearchId]
        )
