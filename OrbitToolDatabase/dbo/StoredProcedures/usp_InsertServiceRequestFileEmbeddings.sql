/*
    Stored Procedure: usp_InsertServiceRequestFileEmbeddings
    Description: Inserts a new service request file embedding record with deduplication.
    Uses BinaryEmbeddingsStore for deduplicated binary storage.
*/
CREATE PROCEDURE [dbo].[usp_InsertServiceRequestFileEmbeddings]
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (
            ...
            WHERE [ServiceRequestFileId] = @ServiceRequestFileId
        ...
        INSERT INTO [dbo].[ServiceRequestFileEmbeddings] (
            [ServiceRequestFileId],
            [BinaryEmbeddingsStoreId],
            [Name],
            [FileHash],
        ...
        -- Build notes
        SET @Notes = CONCAT('Request file embedding created for: ', @Name, 
        ...
        EXEC [dbo].[usp_InsertUserActivities]
            @UserId = @ResolvedUser,
            @ActivityType = 'ServiceRequestFileEmbeddingCreate',
            @ActionType = 'Create',
    ...
    BEGIN CATCH
        ...
        RAISERROR('Error creating request file embedding: %s', @ErrorSeverity, @ErrorState, @ErrorMessage);
    END CATCH
END;
GO