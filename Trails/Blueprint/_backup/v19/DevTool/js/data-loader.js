/**
 * DevBizToolsSuite - Shared Data Loader Module
 ...
 *
 * This file is loaded BEFORE common.js so that DATA is available
 * when modules initialize.
 */
...
/**
 * Get a value from the DATA store by path, with optional fallback.
 * @param {string} key - Top-level key in DATA
 * @param {string} [subPath] - Optional dotted path for nested access (e.g. "soapActions.0.name")
 * @param {*} [fallback] - Fallback value if not found
 * @returns {*}
 */
function getData(key, subPath, fallback) {
    const root = DATA[key];
    if (root === undefined || root === null) return fallback;
    if (!subPath) return root;

    return subPath.split('.').reduce((acc, segment) => {
        if (acc && acc[segment] !== undefined) return acc[segment];
        return fallback;
    }, root);
}
