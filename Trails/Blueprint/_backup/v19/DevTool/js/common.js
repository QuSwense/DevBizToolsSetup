...
function toggleTheme() {
    ...
    localStorage.setItem('devbiztools.mode', newTheme);

    // Re-apply palette if the function exists (defined by palette module)
    if (typeof applyPalette === 'function') {
        const activePalette = htmlEl.getAttribute('data-theme-palette') || 'bootstrap-default';
        applyPalette(activePalette, false);
    }

    // Update Monaco editor theme
    if (window.monaco) {
        const theme = newTheme === 'dark' ? 'corporate-dark' : 'corporate-light';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Restore theme
    const savedMode = localStorage.getItem('devbiztools.mode');
