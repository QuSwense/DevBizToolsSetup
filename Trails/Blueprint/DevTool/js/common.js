...
function parseHash() {
    ...
    const validModules = ['dashboard', 'soap-api-test', 'rest-api-test', 'test-suites', 'pdf-viewer', 'health-checks', 'settings'];
...
/**
 * Load saved font settings from AppStorage and apply to document root.
 * This runs on every page load so fonts are consistent across all modules.
 */
function loadGlobalFontSettings() {
    const saved = AppStorage.getObject(APP.STORAGE_KEYS.FONT);
    if (!saved) return;
...
function applySidebarState(collapsed, persist = true) {
    ...
    if (persist) {
        AppStorage.set(APP.STORAGE_KEYS.SIDEBAR_COLLAPSED, collapsed ? '1' : '0');
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // Initialise the namespaced storage utility
    AppStorage.init(APP.NAMESPACE);

    // Restore sidebar state
    const savedSidebar = AppStorage.get(APP.STORAGE_KEYS.SIDEBAR_COLLAPSED);
