...
(function () {
    ...
    function loadData() {
        ...
        // Restore gradient mode checkbox from storage.
        // Default to checked (true) since CSS default :root has a gradient.
        var savedGradient = AppStorage.get(APP.STORAGE_KEYS.SETTINGS_GRADIENT);
        var gradientCheckbox = document.getElementById('settingsGradientMode');
        if (gradientCheckbox) {
            gradientCheckbox.checked = savedGradient !== null ? savedGradient === 'true' : APP.DEFAULTS.SETTINGS_GRADIENT;
        }
        ...
        // Restore lighter palette checkbox from storage.
        // Default to checked (true) so UI uses the lighter/softer variant.
        var savedLighter = AppStorage.get(APP.STORAGE_KEYS.SETTINGS_LIGHTER_PALETTE);
        var lighterCheckbox = document.getElementById('settingsLighterPalette');
        if (lighterCheckbox) {
            lighterCheckbox.checked = savedLighter !== null ? savedLighter === 'true' : APP.DEFAULTS.SETTINGS_LIGHTER_PALETTE;
        }

        // Restore theme mode from storage and update both the toggle and the label.
        var savedTheme = AppStorage.get(APP.STORAGE_KEYS.SETTINGS_THEME);
    ...
    function applyPalette() {
        ...
        AppStorage.set(APP.STORAGE_KEYS.SETTINGS_PALETTE, key);
        AppStorage.set(APP.STORAGE_KEYS.SETTINGS_GRADIENT, String(useGradient));
        AppStorage.set(APP.STORAGE_KEYS.SETTINGS_LIGHTER_PALETTE, String(useLighter));
    ...
    function resetPalette(useGradient) {
        ...
        AppStorage.remove(APP.STORAGE_KEYS.SETTINGS_PALETTE);
        AppStorage.remove(APP.STORAGE_KEYS.SETTINGS_GRADIENT);
        AppStorage.remove(APP.STORAGE_KEYS.SETTINGS_LIGHTER_PALETTE);
    ...
    /**
     * Load saved font settings from AppStorage and populate UI controls.
     */
    function loadFontSettings() {
        var saved = AppStorage.getObject(APP.STORAGE_KEYS.FONT) || {};
    ...
    /**
     * Apply font settings from the UI controls and persist to AppStorage.
     */
    function applyFontSettings() {
        ...
        // Persist to storage
        AppStorage.setObject(APP.STORAGE_KEYS.FONT, {
            fontFamily: family,
     ...
     * Updates data-bs-theme on <html>, persists via AppStorage, syncs the UI,
    ...
    function toggleTheme() {
        ...
        AppStorage.set(APP.STORAGE_KEYS.SETTINGS_THEME, newTheme);
